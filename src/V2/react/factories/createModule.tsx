import type { ComponentType, JSX, ReactNode } from "react"

import type { ModuleResolutionParams } from "../../core/module/resolution.types.js"
import { ModuleProvider } from "../providers/ModuleProvider.js"
import { usePropsRef, type UsePropsRefOptions } from "../hooks/usePropsRef.js"

export type CreateModuleParams<P> = ModuleResolutionParams | ((props: P) => ModuleResolutionParams)

export type CreateModuleOptions<P extends object, T = P> = UsePropsRefOptions<P, T>

export function createModule<P extends object = {}, T = P>(
    params?: CreateModuleParams<P>,
    options?: CreateModuleOptions<P, T>
): ComponentType<P & { children?: ReactNode }> {
    function Module(props: P & { children?: ReactNode }): JSX.Element {
        const { children, ...ownProps } = props

        // Auto-bridge: every createModule module registers a component-owned PropsRef automatically.
        const { provider } = usePropsRef(ownProps as P, options)
        const resolvedParams = typeof params === "function" ? params(ownProps as P) : params
        const providers = [provider, ...(resolvedParams?.providers ?? [])]

        const moduleParams = { ...(resolvedParams ?? {}), providers } as ModuleResolutionParams

        return <ModuleProvider {...moduleParams}>{children}</ModuleProvider>
    }

    Module.displayName = "Module"

    return Module
}
