...
// ========================================
//
// `npm run test:memory`. Standalone by design — `node tests/memory/run-memory-tests.mjs` works too, and
...
const args = isWindows ? ["/d", "/s", "/c", `npm exec -- ${vitest.join(" ")}`] : ["exec", "--", ...vitest]
