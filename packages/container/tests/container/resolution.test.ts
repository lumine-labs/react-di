...
import { RegistrationMode, ResolveAllMode, ResolveMode, Scope } from "../../src/container.types.js"
import type { Constructor } from "../../src/container.types.js"
import { injectAll } from "../../src/injector.js"
...
//   nearest  — the first binding at or above this container; for a collection, the nearest CONTRIBUTING
//              container's bindings alone.
...
describe("resolveAll", () => {
    ...
    it("falls back to the nearest contributing ancestor in nearest mode", () => {
        ...
        // Owner ruling 2026-08-01, carried over from the inversify-backed original where this shape was
        // MEASURED against the substrate (probe-multiprovider-2-getall-chain 2h): `nearest` conforms to an
        // unchained collection read rather than guarding against one. Here it is our own definition, and it
        // is what lets Container, Resolver, useResolveAll and `injectAll` agree on one vocabulary.
...
// ========================================
//
// The two multi-injection paths must not disagree — the same token reached from inside a construction frame
// and through the container has to yield the same set, or "all the plugins" means one thing in a service and
// another in a hook. `injectAll` therefore defaults to `chained`, exactly as `resolveAll` does; an unchained
// default is what would diverge once a token is declared in both a module and an ancestor. These tests
...
//
// `injectAll` reads the DECLARING container of the binding being constructed. For `c.register(Collector)`
// followed by `c.resolve(Collector)` that container is `c` itself, which is why every assertion below can
// compare the injected result against `<same container>.resolveAll(token)`.

describe("injectAll", () => {
    /** A plain consumer — no decorators, the collection arrives through a field initializer. */
    ...
    it("agrees with resolveAll from a container part-way up the chain", () => {
        ...
        // An unchained default would have handed back just ["middle"] here — this is the divergence.
